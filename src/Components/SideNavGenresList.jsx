...
const SideNavGenresList = ({ selectedGenreId }) => {
  ...
  return (
    <div>
        ...
        {genres.map((item,index) => (
            <div 
                key={index} 
                className={`flex gap-2 items-center cursor-pointer group transition-all
                duration-300 rounded-lg p-3  ${activeIndex==index?"bg-slate-300 dark:bg-gray-700 ":null}`}
            ...
            >
                ...
                <h3 
                    ...
                    transition-all duration-300 ${activeIndex==index?"font-bold":null}`}
                >
                    {item.name}
                </h3>
            </div>
